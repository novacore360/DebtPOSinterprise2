...
export default function CustomerPasswords({ customers, updateCustomer, onBack }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'var(--page-bg)', zIndex:1000, overflowY:'auto' }}>
      <div style={{ maxWidth:720, margin:'0 auto', padding:'16px 16px 40px' }}>
        {/* Header with back navigation */}
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
          <button onClick={onBack} style={{ background:'var(--wa-06)', border:'1px solid var(--wa-1)', borderRadius:8, padding:'8px 10px', color:'var(--text-primary)', cursor:'pointer', display:'flex', alignItems:'center', gap:6, fontSize:12, fontWeight:600 }}>
            <ArrowLeft size={16} /> Back
          </button>
          <div>
            <h2 style={{ color:'var(--text-primary)', margin:0, fontSize:18, fontWeight:700 }}>Customer Passwords</h2>
            <p style={{ color:'var(--wa-4)', margin:'2px 0 0', fontSize:12 }}>
        ...
        <button
          ...
          style={{
            ...
            background: missingCount === 0 ? 'var(--wa-05)' : 'linear-gradient(135deg,#4e73df,#224abe)',
            border:'none', borderRadius:9,
            color: missingCount === 0 ? 'var(--wa-3)' : 'var(--text-primary)',
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} style={{ background:'var(--wa-03)', border:'1px solid var(--wa-07)', borderRadius:12, padding:'12px 14px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ color:'var(--wa-4)', fontSize:11 }}>{c.phone || c.email || 'No contact'}</div>
                </div>
                ...
                {!isEditing ? (
                  <>
                    <div style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(78,115,223,0.1)', border:'1px solid rgba(78,115,223,0.25)', borderRadius:8, padding:'6px 10px' }}>
                      ...
                      <span style={{ color:'var(--text-primary)', fontWeight:700, fontSize:14, letterSpacing:1, fontFamily:'monospace' }}>
                        {hasPassword(c) ? String(c.userPass) : '—'}
                      </span>
                    </div>
                    <button onClick={() => startEdit(c)} style={{ padding:'7px', background:'var(--wa-06)', border:'1px solid var(--wa-1)', borderRadius:8, color:'var(--wa-7)', cursor:'pointer', display:'flex', alignItems:'center' }}>
                      <Edit2 size={12} />
                    </button>
                  </>
                ) : (
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    <div style={{ display:'flex', gap:6 }}>
                      <input
                        ...
                        maxLength={5}
                        autoFocus
                        style={{ width:90, padding:'7px 9px', background:'var(--wa-06)', border:'1px solid #4e73df', borderRadius:7, color:'var(--text-primary)', fontSize:14, textAlign:'center', letterSpacing:1, fontFamily:'monospace', outline:'none' }}
                      />
