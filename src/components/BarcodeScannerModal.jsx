...
export default function BarcodeScannerModal({ isOpen, onClose, onDetected, title = 'Scan Barcode' }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: 'var(--nav-bg)', borderRadius: 16, padding: 24,
        width: '100%', maxWidth: 480, border: '1px solid var(--wa-1)',
        boxShadow: '0 20px 60px rgba(0,0,0,0.5)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Camera size={20} color="#4e73df" />
            <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 16 }}>{title}</span>
          </div>
          <button onClick={onClose} style={{
            background: 'var(--wa-1)', border: 'none', borderRadius: 8,
            color: 'var(--text-primary)', cursor: 'pointer', padding: '6px 8px', display: 'flex', alignItems: 'center',
          }}>
            <X size={18} />
          </button>
        </div>
        ...
        <div style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', background: '#000', aspectRatio: '4/3' }}>
          ...
          {!scanning && !error && (
            <div style={{
              ...
              <p style={{ color: 'var(--wa-5)', margin: 0 }}>Initializing camera…</p>
            </div>
          )}
        </div>

        <p style={{ color: 'var(--wa-5)', textAlign: 'center', marginTop: 12, fontSize: 13, marginBottom: 0 }}>
