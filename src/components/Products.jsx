...
const iStyle = { width:'100%', padding:'10px 12px', background:'var(--wa-06)', border:'1px solid var(--wa-1)', borderRadius:9, color:'var(--text-primary)', fontSize:14, outline:'none', boxSizing:'border-box', transition:'border-color 0.2s' };
...
const card = { background:'var(--wa-03)', border:'1px solid var(--wa-07)', borderRadius:14, padding:18 };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10 }}>
        <div>
          <h2 style={{ color:'var(--text-primary)', margin:0, fontSize:18, fontWeight:700 }}>Products</h2>
          <p style={{ color:'var(--wa-4)', margin:'2px 0 0', fontSize:12 }}>{products.length} items in inventory</p>
        </div>
        <button onClick={() => { cancelEdit(); setShowForm(v=>!v); }} style={{ padding:'9px 16px', background:'linear-gradient(135deg,#4e73df,#224abe)', border:'none', borderRadius:9, color:'var(--text-primary)', fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:7, fontSize:13 }}>
      ...
      {showForm && (
        <div style={card}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <p style={{ color:'var(--text-primary)', fontWeight:700, margin:0, fontSize:14 }}>{editing ? 'Edit Product' : 'New Product'}</p>
            <button onClick={cancelEdit} style={{ background:'none', border:'none', color:'var(--wa-4)', cursor:'pointer' }}><X size={16} /></button>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display:'flex', gap:9, marginTop:14 }}>
              <button type="submit" disabled={saving} style={{ flex:1, padding:'11px', background: saving?'rgba(78,115,223,0.4)':'linear-gradient(135deg,#4e73df,#224abe)', border:'none', borderRadius:9, color:'var(--text-primary)', fontWeight:700, cursor: saving?'not-allowed':'pointer', fontSize:14 }}>
                {saving ? 'Saving…' : editing ? 'Save Changes' : 'Add Product'}
              </button>
              {editing && (
                <button type="button" onClick={cancelEdit} style={{ padding:'11px 18px', background:'var(--wa-08)', border:'none', borderRadius:9, color:'var(--text-primary)', fontWeight:600, cursor:'pointer', fontSize:14 }}>
                  Cancel
                </button>
              )}
            </div>
      ...
      {/* Products list */}
      {filtered.length === 0 ? (
        <div style={{ textAlign:'center', padding:'40px 20px', color:'var(--wa-2)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(260px, 1fr))', gap:12 }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ background:'var(--wa-03)', border:`1px solid ${isOut?'rgba(231,74,59,0.3)':isLow?'rgba(246,194,62,0.2)':'var(--wa-07)'}`, borderRadius:12, padding:14 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8, marginBottom:8 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ color:'var(--text-primary)', fontWeight:700, fontSize:14, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.name}</div>
                    <div style={{ color:'var(--wa-35)', fontSize:11, marginTop:2 }}>{p.productCode} {p.category && `· ${p.category}`}</div>
                  </div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:6 }}>
                  <div>
                    <div style={{ color:'var(--wa-4)', fontSize:10, textTransform:'uppercase' }}>Retail</div>
                    <div style={{ color:'#1cc88a', fontWeight:700, fontSize:15 }}>₱{(p.retailPrice||p.price||0).toFixed(2)}</div>
                  </div>
                  <div>
                    <div style={{ color:'var(--wa-4)', fontSize:10, textTransform:'uppercase' }}>Cost</div>
                    <div style={{ color:'var(--wa-6)', fontWeight:600, fontSize:13 }}>₱{(p.costPrice||0).toFixed(2)}</div>
                  </div>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ color:'var(--wa-4)', fontSize:10, textTransform:'uppercase' }}>Stock</div>
