...
import { Download, Upload, Database, User, LogOut, TrendingUp, Infinity, AlertCircle, Bluetooth, BluetoothConnected, BluetoothOff, Printer } from 'lucide-react';
    ...
    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:16 }}>
      ...
      <div style={card}>
        ...
        {!bluetoothSupported ? (
          ...
          <>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14, padding:'9px 12px', borderRadius:9, background: printerStatus==='connected' ? 'rgba(28,200,138,0.1)' : 'rgba(255,255,255,0.04)' }}>
              {printerStatus === 'connected' ? <BluetoothConnected size={16} color="#1cc88a" /> : printerStatus === 'connecting' ? <Bluetooth size={16} color="#f6c23e" /> : <BluetoothOff size={16} color="rgba(255,255,255,0.4)" />}
              <span style={{ fontSize:13, color: printerStatus==='connected' ? '#1cc88a' : 'rgba(255,255,255,0.55)' }}>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:14 }}>
              {printerStatus === 'connected' ? (
                ...
                <button onClick={connectPrinter} disabled={printerStatus==='connecting'} style={{ flex:1, padding:'10px 12px', background:'linear-gradient(135deg,#4e73df,#224abe)', border:'none', borderRadius:9, color:'#fff', cursor: printerStatus==='connecting'?'not-allowed':'pointer', fontWeight:600, fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
                  <Bluetooth size={14} /> {printerStatus==='connecting' ? 'Connecting…' : 'Pair Printer'}
                </button>
              )}
              <button onClick={testPrint} disabled={testPrinting} style={{ flex:1, padding:'10px 12px', background:'rgba(255,255,255,0.08)', border:'none', borderRadius:9, color:'#fff', cursor: testPrinting?'not-allowed':'pointer', fontWeight:600, fontSize:13 }}>
            ...
            <p style={{ color:'rgba(255,255,255,0.35)', fontSize:11, marginBottom:14, lineHeight:1.5 }}>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:14 }}>
              <p style={{ color:'rgba(255,255,255,0.5)', fontSize:12, fontWeight:600, marginBottom:10, textTransform:'uppercase', letterSpacing:.4 }}>Receipt Details</p>
