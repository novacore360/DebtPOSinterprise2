  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10 }}>
        <div>
          <h2 style={{ color:'var(--text-primary)', margin:0, fontSize:18, fontWeight:700 }}>Customers</h2>
          <p style={{ color:'var(--wa-4)', margin:'2px 0 0', fontSize:12 }}>{customers.length} registered customers</p>
        </div>
      ...
      <div style={{ position:'relative' }}>
        <Search size={14} style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'var(--wa-35)' }} />
        <input
          value={search}
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'var(--nav-bg)', border:'1px solid var(--wa-12)', borderRadius:9, overflow:'hidden', zIndex:20, boxShadow:'0 10px 24px rgba(0,0,0,0.45)' }}>
            {suggestions.map(c => (
              <div
                key={c.id}
                onMouseDown={() => { setSearch(c.name || ''); setShowSuggestions(false); }}
                style={{ padding:'9px 12px', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid var(--wa-05)' }}
              >
                <span style={{ color:'var(--text-primary)', fontSize:13 }}>{c.name}</span>
                <span style={{ color:'var(--wa-35)', fontSize:11 }}>{c.phone || ''}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.length === 0 && (
          <div style={{ textAlign:'center', padding:'32px 16px', color:'var(--wa-2)', fontSize:14 }}>No customers found.</div>
        )}
        {filtered.map(c => {
              ...
              <div onClick={() => { setExpandedId(isOpen ? null : c.id); setEditMode(false); }}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0 }}>
                  <div style={{ width:36, height:36, borderRadius:'50%', background:'linear-gradient(135deg,rgba(78,115,223,0.3),rgba(28,200,138,0.3))', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <span style={{ color:'var(--text-primary)', fontWeight:800, fontSize:14 }}>{c.name?.[0]?.toUpperCase()}</span>
                  </div>
                  <div style={{ minWidth:0 }}>
                    <div style={{ color:'var(--text-primary)', fontWeight:700, fontSize:14, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.name}</div>
                    <div style={{ color:'var(--wa-4)', fontSize:11, marginTop:1 }}>{c.phone||c.email||'No contact'}</div>
                  </div>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:12, flexShrink:0 }}>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ color:'#1cc88a', fontWeight:700, fontSize:13 }}>₱{spent.toFixed(2)}</div>
                    <div style={{ color:'var(--wa-3)', fontSize:11 }}>{cPurchases.length} orders</div>
                  </div>
                  {isOpen ? <ChevronUp size={14} color="var(--wa-4)" /> : <ChevronDown size={14} color="var(--wa-4)" />}
                </div>
              </div>

              {/* Expanded */}
              {isOpen && selected && (
                <div style={{ borderTop:'1px solid var(--wa-07)', padding:14 }}>
                  {!editMode ? (
                    <>
                      ...
                      {/* Purchases list */}
                      <p style={{ color:'var(--wa-5)', fontSize:12, fontWeight:600, marginBottom:8, textTransform:'uppercase', letterSpacing:.4 }}>Purchase History</p>
                      {custPurchases.length === 0 ? (
                        <p style={{ color:'var(--wa-2)', fontSize:13, textAlign:'center', padding:'16px 0' }}>No purchases yet.</p>
                      ) : (
                        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                          {custPurchases.map(p => (
                            <div key={p.id} style={{ background:'var(--wa-04)', borderRadius:10, padding:'10px 12px' }}>
                              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:8 }}>
                                <div>
                                  <div style={{ color:'var(--wa-4)', fontSize:11 }}>{new Date(p.purchase_date).toLocaleString()}</div>
                                  <div style={{ color:'var(--wa-7)', fontSize:13, marginTop:3 }}>
                          ...
                          <div style={{ borderTop:'1px solid var(--wa-07)', paddingTop:8, display:'flex', justifyContent:'space-between' }}>
                            <span style={{ color:'var(--wa-5)', fontSize:13 }}>Total Spent</span>
                    ...
                    <div>
                      <p style={{ color:'var(--text-primary)', fontWeight:700, margin:'0 0 12px', fontSize:14 }}>Edit Customer</p>
                      ...
                      <div style={{ display:'flex', gap:8 }}>
                        <button onClick={saveEdit} style={{ flex:1, padding:10, background:'linear-gradient(135deg,#4e73df,#224abe)', border:'none', borderRadius:8, color:'var(--text-primary)', fontWeight:700, cursor:'pointer', fontSize:14 }}>Save</button>
                        <button onClick={() => setEditMode(false)} style={{ padding:'10px 14px', background:'var(--wa-08)', border:'none', borderRadius:8, color:'var(--text-primary)', cursor:'pointer', fontSize:14 }}>Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
